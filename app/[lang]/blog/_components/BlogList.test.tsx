...
describe("BlogList", () => {
  it("renders one Card per post", () => {
    ...
    render(<BlogList blogPosts={query} />);
    expect(screen.getAllByTestId("card")).toHaveLength(2);
  });

  it("renders nothing when there are no posts", () => {
    render(<BlogList blogPosts={createBlogQuery([])} />);
  ...
  it("passes the meta_image to Card", () => {
    render(<BlogList blogPosts={createBlogQuery([createBlogPost()])} />);
  ...
  it("falls back to 'Uncategorized' when the category relationship is unfilled", () => {
    ...
    render(<BlogList blogPosts={createBlogQuery([post])} />);
